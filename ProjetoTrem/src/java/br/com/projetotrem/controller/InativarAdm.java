    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                if (dao.inativar(administrador)) {
                    menssagem = "Administrador inativado com sucesso.";
                    request.setAttribute("menssagem", menssagem);
                    request.getRequestDispatcher("ListarAdmSisAt").forward(request, response);
                } else {
                    menssagem = "Problemas ao inativar Cliente.";
                    request.setAttribute("menssagem", menssagem);
                    request.getRequestDispatcher("ListarAdmSisAt").forward(request, response);
                }
            } catch (Exception e) {
                System.out.println("Problemas no Servlet ao Inativar Erro: "
                        + e.getMessage());
                e.printStackTrace();
