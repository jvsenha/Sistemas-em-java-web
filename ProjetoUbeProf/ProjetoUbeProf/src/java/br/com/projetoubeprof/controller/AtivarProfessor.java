 ...
 * @author AlunoRemoto
 */
@WebServlet(name = "AtivarProfessor", urlPatterns = {"/AtivarProfessor"})
public class AtivarProfessor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                if (dao.ativar(professor)) {
                    msgLp = "Professor ativado com sucesso.";
                    request.setAttribute("msgLp", msgLp);
                    request.getRequestDispatcher("ListarProfessor").forward(request, response);
                } else {
                    msgLp = "Problemas ao inativar Cliente.";
                    request.setAttribute("msgLp", msgLp);
                    request.getRequestDispatcher("ListarProfessor").forward(request, response);
                }
            } catch (Exception e) {
