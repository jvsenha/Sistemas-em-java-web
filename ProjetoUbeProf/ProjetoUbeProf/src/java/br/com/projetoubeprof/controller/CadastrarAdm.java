    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String mensagemadm= null;
              ...
              administrador.setStatusUsuario(statusUsuario);
    
               try {
                    GenericDAO dao = new AdministradorDAOImpl();
                    if (dao.cadastrar(administrador)) {
                        mensagemadm = "Administrador cadastrado com sucesso!";
                        request.setAttribute("mensagemadm", mensagemadm);
                    request.getRequestDispatcher("homeadm.jsp").forward(request, response);
              
                    } else {
                        mensagemadm = "Problemas ao cadastrar Cliente. "
                                + "Verifique os dados informados e tente novamente!";
                     request.setAttribute("mensagemadm", mensagemadm);
                    request.getRequestDispatcher("adm/cadastraradm.jsp").forward(request, response);
